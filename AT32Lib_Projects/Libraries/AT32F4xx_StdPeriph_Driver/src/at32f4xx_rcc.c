...
/**
  ...
  *     @arg RCC_CLKOUT_AHB:      AHB clock selected
  *     @arg RCC_CLKOUT_APB1:     APB1 clock selected
  *     @arg RCC_CLKOUT_APB2:     APB2 clock selected
  *     @arg RCC_CLKOUT_TIM1:     TIM1 clock selected
  *     @arg RCC_CLKOUT_TIM2:     TIM2 clock selected
  *
  * @retval None
  */
...
#ifdef AT32F413xx
		else if((RCC_CLKOUT >> 28) == 2)
		{
			// Set CLKOUT[2:0] = 0x1, use CLKOUT_SEL;
			RCC->MISC &= ~RCC_MISC_CLKOUT_3;

			RCC->CFG &= ~RCC_CFG_CLKOUT;
			RCC->CFG |= RCC_CFG_CLKOUT_0;

			/* Set CLKOUT_SEL bits according to RCC_CLKOUT value */
			tmpreg = RCC->TEST;
			/* Clear CLKOUT_SEL[2:0] bits */
			tmpreg &= ~RCC_TEST_CLKOUT_SEL;
			/* Set CLKOUT_SEL[2:0] bits */
			tmpreg |= (RCC_CLKOUT & 0xFFFFFFF);
			/* Store the new value */
			RCC->TEST = tmpreg;
		}
...
void RCC_StepModeCmd(FunctionalState NewState)
{
  ...
  if(ENABLE == NewState)
	{
    RCC->MISC2 |= RCC_MISC2_AUTO_STEP_MODE;
	}
	else
	{
    RCC->MISC2 &= ~RCC_MISC2_AUTO_STEP_MODE;
...
void RCC_USBINTRemap(FunctionalState NewState)
{
  ...
  if(ENABLE == NewState)
	{
    RCC->INTCTRL |= RCC_INTCTRL_USB_INT_REMAP;
	}
	else
	{
    RCC->INTCTRL &= ~RCC_INTCTRL_USB_INT_REMAP;
