...
/**
  ...
  * @note   This function can be used for all AT32F41x devices.
  * @param  Psw: SLIB Password
  *         StartPage:SLIB Start Page (N: 0~127)
  *         DataPage: SLIB data Start Page (M: 0~127)
  *         EndPage:  SLIB End Page (C: 0~127)
...
FLASH_Status FLASH_SlibMainEnable(uint32_t Psw, uint16_t StartPage, uint16_t DataStartPage, uint16_t EndPage)
{
  ...
  assert_param(StartPage <= 127); 
  assert_param(DataStartPage <= 127);
  assert_param(EndPage <= 127);
 ...
 * @retval uint16_t
 */
uint16_t SWD_F413_FLASH_GetSlibEndPage(void)
{
