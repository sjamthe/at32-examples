...
/**
  ...
  * @retval None
  */
void AT32_Hoverboard_Init()
{
    /*initialize Delay Function*/
    Delay_init();

    /* hold board power pin high */
    Board_Init();
    ...
    AT32_BUTTON_Init(BUTTON_WAKEUP); /*PA5*/
...
void Board_Init()
{
    ...
    RCC_APB2PeriphClockCmd(POWER_RCC_CLK, ENABLE);

    /*Configure Button pin as input with pull-up/pull-down*/
    GPIO_StructInit(&GPIO_InitStructure);
    GPIO_InitStructure.GPIO_Pins = POWER_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_PU; //hold high for board to stay in power on mode
...
void UART_Print_Init(uint32_t bound)
{
    ...
    GPIO_InitStructure.GPIO_Pins = AT32_PRINT_UARTRX_PIN;//PA3
